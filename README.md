...
- `engine/annual.mjs` — 연차 유급휴가·연차수당(근로기준법 제60조: 1년 미만 개근 1개월당 1일 최대 11일, 1년 이상 15일 + 3년째부터 2년마다 1일 한도 25일, 회계연도 기준 첫해 비례연차, 1일 통상임금 = 월 통상임금 ÷ 209 × 8) → `/annual/{근속 년}/` 25장 + `/annual/pay/{월 통상임금 만원}/` 17장 + `/annual/` 계산기
- `engine/freelance.mjs` — 프리랜서 원천징수(사업소득 3.3% = 소득세 3% + 지방소득세 0.3%, 기타소득 8.8% = 필요경비 60% 인정 후 22%, 건당 12만 5천원 이하 과세최저한, 실수령→세전 역산) → `/freelance/{금액 만원}/` 20장 + `/freelance/` 계산기
- `engine/nhis.mjs` — 건강보험료(2025년 기준: 직장 보수월액 × 7.09% 노사 반반·상한 1,272만·하한 279,300원, 지역 소득 정률 7.09%·최저보험료 19,780원·재산 부과점수 × 208.4원, 장기요양 12.95%). **지역가입자 재산 점수는 공단 60등급표를 6단계로 줄인 근사** — 정확한 금액은 공단 모의계산(nhis.or.kr) → `/nhis/employee/{보수월액 만원}/` 17장 + `/nhis/local/{연소득 만원}/` 12장 + `/nhis/` 계산기
...
- 1월: 건강보험료율·장기요양보험료율·보수월액 상한/하한·지역가입자 부과점수당 금액과 최저보험료(국민건강보험공단 고시) — `engine/nhis.mjs`
